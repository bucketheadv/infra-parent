...
- Admin 集群必须连接同一个 MySQL 8+ 数据库，且每个节点配置可识别的 `infra.schedule.scheduler-id`。实际租约 owner 会在每次进程启动时追加随机 token，保证旧进程重启后不能误续租新进程的记录。
...
4. Outbox 投递循环以独立租约领取已到达可领取时间的 `PENDING` 记录，异步提交到本节点工作线程。工作线程被拒绝、调度异常或执行器调用结果未知时，记录会回到 `PENDING` 并按已尝试次数指数退避（1、2、4…秒，最多 60 秒）；`claim_until` 在 `PROCESSING` 状态下为租约截止时间、在 `PENDING` 状态下为下一次可领取时间，防止故障执行器每秒被重复投递。
...
手动执行也先锁定任务行并写入带 `manual_trigger` 标记的 Outbox，不会绕过可靠投递路径，也不会改变 Cron 或固定频率的下一次计划时间。暂停仅停止定时扫描，仍可立即执行一次；暂停或编辑会取消未开始的定时 Outbox，删除任务才会一并取消未开始的手动 Outbox。已经提交到工作线程或执行器的执行会继续走取消流程或自然结束。

任务编辑、停用和删除会锁定任务行，并在同一 MySQL 事务内变更任务定义与撤销未开始的 Outbox。重复提交同一启用或停用状态是幂等操作：状态已经一致时保留已有调度进度和活跃 Outbox，不会因为管理接口重放而取消正在投递的触发。执行器按同一内部状态锁原子处理队列出队、运行标记与按 `logId` 取消：取消请求要么撤销尚未开始的票据，要么中断已经标记运行的 Handler。删除流程首先原子停用任务，随后取消活跃日志并发出精确取消请求，最后删除定义；停用与删除之间不会再创建新的执行日志。

当前 Outbox 保障“计划推进与待投递记录”在一个事务内完成，并支持投递租约恢复。它不是端到端 exactly-once 协议：执行器侧业务处理必须按任务参数或业务键实现幂等，以承受网络超时、进程崩溃和可能的重复投递。一次触发的每次**确认失败**的远程重试都会创建独立执行日志和独立 `logId`；Outbox 仍表示整次触发，管理端按日志 ID 分别处理 started、finish、cancel 和探活，避免旧尝试回调覆盖新尝试。HTTP 连接中断、读超时和 5xx 等结果未知时，调度中心先按该 `logId` 探活：确认仍在运行则确认 Outbox 已受理，确认不存在则按重试策略处理，仍未知才保留活跃日志并将 Outbox 释放回 `PENDING`，按 1、2、4…秒（最长 60 秒）的退避后至少一次重投。HTTP 4xx 为执行器明确拒绝，按确定失败处理。
...
执行器被禁用后不再参与任务选择。没有候选节点或执行器明确拒绝时会写入失败执行日志；`FAILOVER` / `BUSYOVER` 的探活只要存在网络或协议未知节点，就保留 Outbox 并按退避重试，不能把暂时不可达误判为确定失败。上述处理不会阻塞其他任务。
...
执行日志的活跃状态为 `QUEUED`、`RUNNING`、`CANCELLING`、`TIMING_OUT`；终态包括 `SUCCESS`、`FAILED`、`TIMEOUT`、`CANCELLED`、`SKIPPED` 和 `LOST`。管理员取消或执行超时不会立即掩盖远端状态：日志先分别进入 `CANCELLING` / `TIMING_OUT`，继续按 `logId` 探活；仅在执行器回调或探活明确确认任务已退出后，才收口为 `CANCELLED` / `TIMEOUT`。

任务显式配置 `timeoutSeconds` 时优先使用该值；未配置时使用 `max-execution-millis`（默认 1 小时）。超时时调度中心会取消本地等待 Future，并向远程执行器请求 `/cancel`；管理员取消或删除任务时也会发起同样的取消请求。网络失败或 Handler 忽略中断时，取消确认状态会保留，避免仍在运行的远端任务从系统视野消失；确认仍在运行的取消确认日志会按最短 5 秒间隔重发精确 `/cancel` 并继续探活。

每轮扫描会回收僵尸活跃日志：

1. 候选日志为投递尝试创建时间早于 `max(stale-running-log-millis, claim-lease-millis, 60000ms)` 的 `QUEUED` / `RUNNING` 记录；`CANCELLING` / `TIMING_OUT` 会立即参与有限并发探活和取消补偿，不等待普通僵尸阈值。两类候选分别按日志主键游标分页并回绕，避免最老积压记录造成分页饥饿。
2. 对远程执行器调用 `/running?logId=...`；本地执行器查询 `ExecutorTaskTracker`。
3. 只有执行器明确返回“不存在”时，才将日志更新为 `LOST`；探活采用最多 16 个并发请求和单轮约 5 秒预算，超出预算的候选留待下一轮。
4. 网络不可达、鉴权失败或协议异常属于未知状态，保留活跃日志等待下次探测，避免把实际仍在运行的任务误判为丢失。

`LOST` 是日志状态修正，不会自动杀死远程进程。执行器失联、日志回调失败或进程崩溃应结合心跳、网络监控和业务幂等进行处置。

执行器的 started / finish 回调以及过程日志刷写均提交到独立、有界的单线程回调队列，同一日志按入队顺序发送，不阻塞 JobThread；过程日志存在可恢复错误时，finish 回调会延后，避免终态先于业务日志到达。可恢复网络错误最多异步重试 3 次，普通 4xx 视为永久失败。过程日志遇到普通 4xx（例如日志已被清理的 404）会丢弃当前批次，避免队首无限重试；网络错误、5xx、408 和 429 保留重试。普通回调槽位满时，finish 会按 `logId` 去重保留至最多 1000 条保底记录，并在后续刷写周期重投；保底容量耗尽时记录错误日志，最终由 Admin 僵尸回收纠正活跃状态，避免以无界内存换取可靠性。

## 7. 历史日志清理

每轮调度结束后，系统按 `execution-log-retention-millis` 清理已结束日志，默认保留 30 天；设置为 `0` 可关闭自动清理。

- 仅删除 `finish_time` 早于保留阈值且不再是活跃状态的记录；清理查询使用时间索引并按批删除。
