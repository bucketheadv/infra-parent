...
CREATE TABLE IF NOT EXISTS infra_schedule_job (
    ...
    route_strategy VARCHAR(32) NOT NULL COMMENT '路由策略（对齐 xxl-job）',
    block_strategy VARCHAR(32) NOT NULL COMMENT '阻塞策略：SERIAL/DISCARD_LATER/COVER_EARLY（执行器侧生效）',
    resident TINYINT(1) NOT NULL DEFAULT 0 COMMENT '是否常驻任务',
