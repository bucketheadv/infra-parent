# Infra Market Codex 协作规范

本文件将 `.cursor/rules/` 中的现有规则转换为 Codex 的项目级指令。处理本仓库任务时，遵循以下规范；如用户指令与本文件冲突，以用户指令为准。

## 沟通与改动原则

- 默认使用中文，保持专业、简洁、友好；先给技术结论或可执行方案。若用户明确要求只要技术结论，不添加寒暄或鼓励。
...
## 后端基线（适用于 `infra-market-server/src/main/kotlin/**/*.kt`）

- 后端核心栈为 Kotlin、Spring Boot、MyBatis-Flex、MySQL 和 Maven；遵循可读性、稳定性和 Spring 分层实践。
- `infra-market-server` 的 Kotlin 代码包位于 `io.infra.market` 下，按 `controller`、`service`、`repository`/`dao`、`dto`、`config`、`util` 等职责组织。
...
- 时间计算和格式化使用 Joda Time，不引入 `java.time`。请求与响应使用对象封装，避免散落的原始参数。
- 保持 Kotlin 空安全，避免滥用 `!!`；方法命名应表达业务语义，不保留未使用的方法。发现重复逻辑时优先提取复用方法。

## API、异常与日志（适用于 `infra-market-server/src/main/kotlin/**/*.kt`）
...
## 复杂业务与性能（适用于 `infra-market-server` 与 `infra-market-server-vertx` 的 Kotlin 代码）
...
## Redis（适用于 `infra-market-server/src/main/kotlin/**/*.kt`）
...
## 配置（适用于 `infra-market-server/src/main/resources/**/*.{yml,yaml,properties}`）

- 本地、测试、生产配置必须隔离；默认配置不得包含生产地址。
- 密码、密钥和 Token 不得以明文提交；使用密文或外部密钥管理。
- 超时、重试和连接池大小必须显式配置。新增开关必须提供默认值与回滚策略，并保持配置命名语义一致。

## 测试与发布安全（适用于 `infra-market-server/src/main/kotlin/**/*.kt`）

- 用户要求补测试时优先覆盖核心业务路径；修复线上问题时优先补充可复现问题的回归测试。
- 涉及接口字段、状态机或消息事件的改动，说明兼容策略和灰度风险。

## 接口安全（适用于 `infra-market-server` 与 `infra-market-server-vertx` 的 Kotlin API 变更）

- 按“最不信任调用方”设计。身份必须由服务端会话或 Token（如 `AuthHolder`）解析，禁止将请求体、Query 或 Header 中的 `uid`/`userId` 作为操作主体；代操作需明确授权。无效、过期或校验失败的 Token 一律拒绝。
...
- 变更 Kotlin 后，按受影响模块执行适当的 Maven 编译、测试或最小验证；未运行时说明原因。
